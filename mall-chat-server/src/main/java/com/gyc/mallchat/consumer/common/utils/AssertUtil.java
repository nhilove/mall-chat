...
import com.gyc.mallchat.consumer.common.exception.enums.ErrorEnum;
...
public class AssertUtil {
    ...
    public static <T> void fastFailValidate(T obj) {
        ...
        if (constraintViolations.size() > 0) {
            throwException(CommonErrorEnum.PARAMS_INVALID, constraintViolations.iterator().next().getMessage());
        }
    }
    ...
    public static <T> void allCheckValidateThrow(T obj) {
        ...
        if (constraintViolations.size() > 0) {
            ...
            //去掉最后一个逗号
            throwException(CommonErrorEnum.PARAMS_INVALID, errorMsg.toString().substring(0, errorMsg.length() - 1));
        }
    }
